...
router
    ...
    .post(restrictTo('Admin'), asyncHandler(classesController.linkSubject));

router.get(
    '/:class_id/students',
    resolveProfile,
    restrictTo('Admin', 'Faculty'),
    asyncHandler(classesController.getClassStudents),
);

router.delete(
    '/:class_id/subjects/:subject_id',
    restrictTo('Admin'),
    asyncHandler(classesController.unlinkSubject),
);
