...
exports.createSchedule = async (req, res) => {
    const { title, class_id, subject_id, exam_type, exam_datetime } = req.body;
    if (!title || !class_id || !subject_id || !exam_type || !exam_datetime) {
    ...
    const newSchedule = await examSchedulesService.createSchedule(
        { title, class_id, subject_id, exam_type, exam_datetime },
        createdByUserId,
    );
...
exports.getSchedules = async (req, res) => {
    ...
    const { rows, total } = await examSchedulesService.getSchedules(req.query, pagination, sorting, req.user);
...
exports.getExamTypes = async (req, res) => {
    const types = await examSchedulesService.getExamTypes(req.user);
    return successResponse(res, types);
};

exports.updateSchedule = async (req, res) => {
    const { title, exam_type, exam_datetime } = req.body;
    const actorId = req.user.user_id;
    const updated = await examSchedulesService.updateSchedule(
        req.params.schedule_id,
        { title, exam_type, exam_datetime },
        actorId,
    );
