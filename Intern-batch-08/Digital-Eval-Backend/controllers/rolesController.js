...
exports.assignPermission = async (req, res) => {
    const roleId = req.params.role_id;
    let { permission_id, name } = req.body;

    // Postman sends 'name' instead of 'permission_id' in some docs
    if (!permission_id && name) {
        const permissionsModel = require('../models/permissionsModel');
        const perm = await permissionsModel.findByName(name);
        if (perm) {
    ...
    if (!permission_id)
        return res
            .status(400)
            .json({ success: false, error: { code: 'BAD_REQUEST', message: 'permission_id or valid permission name is required' } });
