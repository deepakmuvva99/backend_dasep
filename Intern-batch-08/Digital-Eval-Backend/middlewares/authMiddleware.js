...
const verifyToken = async (req, res, next) => {
    ...
    try {
        ...
        // 1. CHECK BLACKLIST: Verify if this specific token (jti) has been blacklisted (logged out)
        if (decoded.jti) {
        ...
        // 2. CHECK SESSION ACTIVITY (KICK-OUT LOGIC):
        // We hash the token and check if it is still marked as active in the database.
        // If a new login occurred elsewhere, this session will have been marked as inactive.
        const tokenHash = crypto.createHash('sha256').update(token).digest('hex');
        const isActive = await authModel.isSessionActive(tokenHash);
        if (!isActive) {
            return errorResponse(res, 'UNAUTHORISED', 'You have been logged in on another device. Please login again.', 401);
        }
