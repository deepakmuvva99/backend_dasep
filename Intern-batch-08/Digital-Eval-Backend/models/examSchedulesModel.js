...
class ExamSchedulesModel {
    async createSchedule(data, createdByUserId) {
        const [result] = await db.execute(
            `INSERT INTO exam_schedules (title, class_id, subject_id, exam_type, exam_datetime, created_by_user_id, created_at)
             VALUES (?, ?, ?, ?, ?, ?, NOW())`,
            [data.title, data.class_id, data.subject_id, data.exam_type, data.exam_datetime, createdByUserId],
        );
    ...
    async getSchedules(filters, pagination, sorting, userContext) {
        let query = `
            SELECT es.exam_schedule_id, es.exam_schedule_id as id, es.title, es.exam_datetime, es.created_at,
                   ...
                   te.name as type_name, te.id as exam_type
            FROM exam_schedules es
            JOIN classes c ON es.class_id = c.class_id
            JOIN subjects s ON es.subject_id = s.subject_id
            LEFT JOIN type_of_exam te ON es.exam_type = te.id
        `;
        const params = [];
        const conditions = [];
    ...
    async updateSchedule(scheduleId, data) {
        const [result] = await db.execute(
            `UPDATE exam_schedules SET title = ?, exam_type = ?, exam_datetime = ? WHERE exam_schedule_id = ?`,
            [data.title, data.exam_type, data.exam_datetime, scheduleId],
        );
