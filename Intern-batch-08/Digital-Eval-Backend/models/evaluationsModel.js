...
class EvaluationsModel {
    ...
    async getStatuses() {
        const [rows] = await db.execute(`SELECT evaluation_status_id as id, name FROM evaluation_status`);
        return rows;
    }
    ...
    async findByFacultyId(facultyId, pagination, filters) {
        ...
        const [countRows] = await db.query(`SELECT COUNT(*) as total FROM (${query}) as sub`, params);
        const total = countRows[0].total;
