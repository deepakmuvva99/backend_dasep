...
class AnnotationsModel {
    ...
    async findById(annotationId) {
        const [rows] = await db.execute(
            `SELECT * FROM annotations WHERE annotation_id = ? AND deleted_at IS NULL`,
            [annotationId]
        );
        return rows[0];
    }
    ...
    async updateAnnotation(annotationId, data) {
        const [result] = await db.execute(
            `UPDATE annotations SET 
             ...
             WHERE annotation_id = ?`,
            [
                data.pos_x, data.pos_y, data.pos_width, data.pos_height, 
                JSON.stringify(data.position_data || {}), data.content, 
                data.annotation_type_id, annotationId
            ]
        );
    ...
    async deleteAnnotation(annotationId) {
        const [result] = await db.execute(
            `UPDATE annotations SET deleted_at = NOW() WHERE annotation_id = ?`,
            [annotationId]
        );
        return result.affectedRows;
    }
